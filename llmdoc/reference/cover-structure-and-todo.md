...
- bachelor cover shares secret mark, logo/name image, type label, title,
...
### Undergraduate cover type and title

The undergraduate cover element formerly named `u / cover / thesis` is now
`u / cover / type`, with content generated by `\@@_u_cover_type:`. The helper
selects `proposal` when `\g_@@_opt_proposal_bool` is true and `thesis`
otherwise. The `cover-u` page element list uses `type`.

The undergraduate title element uses `\heiti` with `26bp/32.5bp` metrics, not
`\sffamily`. This matches the oracle: `thuthesis2e` applies CJK-only `\heiti`
inside the bachelor title parbox, so Chinese title text is black CJK text while
Latin title text remains in the current roman/main font. Do not replace this
with `\sffamily`; it makes the English title sans-serif and breaks oracle
parity.

When `\g_@@_opt_chinese_bool` is false, `\@@_u_cover_title:` prints the English
title below the Chinese title and applies only `\@@_english_punctuation:` around
that English title. It does not call the full `\thusetenglish` declaration.
...
- `u/cover/type` replaced the older local `u/cover/thesis` element name and
  dispatches between thesis and proposal labels through `\@@_u_cover_type:`.
- `u/cover/title` uses `\heiti` and `\@@_u_cover_title:` with
  `\@@_box_paragraph_top_to_ht:nn` for the `136bp` fixed-height parbox, plus
  explicit `\prevdepth` preservation and `\baselineskip`/`\lineskiplimit`
  compensation to match the legacy `\parbox` line-spacing contract. In
  English-main mode it prints `title*` below `title` and only switches English
  punctuation around the English title.
...
- [x] Finish `cover-u` parity (bulk layout complete with spacing refinements, see below):
  ...
  - [x] type label (`\ziju{0.3}`, `48bp` bottom-skip), dispatched by
    `\@@_u_cover_type:` between thesis and proposal labels;
  - [x] title element (centered `\parbox[t][136bp]{\linewidth}` via
    `\@@_box_paragraph_top_to_ht:nn` with `\prevdepth` preservation and
    `\baselineskip`/`\lineskiplimit` compensation, `\heiti`,
    `\fontsize{26bp}{32.5bp}`);
  ...
  - [x] info drawing mechanism: forked from graduate Chinese cover
    ...
    `\@@_cover_info:` removed;
  - [x] English-main title behavior (English title below Chinese title, with
    punctuation-only local English handling);
  - [ ] optional co-supervisor label width behavior;
  - [x] bachelor proposal label when `thesis-type = proposal`.
